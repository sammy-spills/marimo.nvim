...
function M.render(bufnr)
  ...
  for _, cell in ipairs(cells) do
    local first_line = lines[cell.start_line] or ""
    vim.api.nvim_buf_set_extmark(bufnr, M.header_ns, cell.start_line - 1, 0, {
      end_row = cell.start_line - 1,
      end_col = math.max(1, #first_line),
      conceal = "",
      virt_text = header_chunks(cell),
      virt_text_pos = "overlay",
      priority = 201,
    })
    if cell.start_line < cell.def_end_line then
      vim.api.nvim_buf_set_extmark(bufnr, M.ns, cell.start_line, 0, {
        end_row = cell.def_end_line,
        end_col = 0,
        conceal_lines = "",
        priority = 200,
      })
